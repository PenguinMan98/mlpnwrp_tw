 ...
 * This access does remove all other players access to the character.
 * /grant <player_name> <character_name>
 * */
class Operation_Transfer extends OperationBase{
	public $operator;
	public $data;
	...
	private function execute(){
		...
		try{
			$cuProvider->transferCharacter($playerName, $characterName); // do the action. May throw errors.
		}catch(Exception $e){
			$this->messages[] = $e->getMessage();
			return "{{ Error }}";
		}
		return "{{ $playerName has been given $characterName!  Congratulations! }}";
	}
}